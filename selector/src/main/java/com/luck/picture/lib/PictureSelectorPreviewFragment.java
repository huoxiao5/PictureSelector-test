...
public class PictureSelectorPreviewFragment extends PictureCommonFragment {
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        tvSelectedDateTime = view.findViewById(R.id.ps_tv_selected_datetime);
        if (tvSelectedDateTime!=null){
            tvSelectedDateTime.setVisibility(config.isDisplayTimeInPreview?View.VISIBLE:View.GONE);
        }
    ...
    private void initViewPagerData(ArrayList<LocalMedia> data) {
        ...
        if (tvSelectedDateTime!=null && config.isDisplayTimeInPreview){
            tvSelectedDateTime.setText(DateUtils.getFormatString2(media.getDateAddedTime()*1000));
            setTvSelectedDateTimeMargin();
        }
    }
    ...
    private void previewFullScreenMode() {
        ...
        if (isAnimInit) {
           // Log.e(TAG, "onSelectedChange: 隐藏了状态栏了。。。。" );
            if (tvSelectedDateTime!=null){
                tvSelectedDateTime.setVisibility(View.GONE);
            }
            showFullScreenStatusBar();
        } else {
           // Log.e(TAG, "onSelectedChange: 显示了状态栏了。。。。" );
            if (tvSelectedDateTime!=null && config.isDisplayTimeInPreview){
    ...
    private void setTvSelectedDateTimeMargin(){
        ConstraintLayout.LayoutParams layoutParams=(ConstraintLayout.LayoutParams)tvSelectedDateTime.getLayoutParams();
        //判断是否需要设置位置
        if (SelectedManager.getSelectCount()>0){
            //需要把位置往上移，marginBottom 70
            layoutParams.bottomMargin =  DensityUtil.dip2px(getContext(),70);
        }else{
            //把位置往下移 marginBottom 15
            layoutParams.bottomMargin =  DensityUtil.dip2px(getContext(),15);
        }
        tvSelectedDateTime.setLayoutParams(layoutParams);
    }
    ...
    private final ViewPager2.OnPageChangeCallback pageChangeCallback = new ViewPager2.OnPageChangeCallback() {
        ...
        public void onPageSelected(int position) {
            ...
            if (mData.size() > position) {
                ...
                Log.e(TAG, "onPageSelected: config.isDisplayTimeInPreview= "+config.isDisplayTimeInPreview+",  tvSelectedDateTime="+tvSelectedDateTime );
                if (config.isDisplayTimeInPreview){
                    if (tvSelectedDateTime!=null){
                        tvSelectedDateTime.setText(DateUtils.getFormatString2(currentMedia.getDateAddedTime()*1000));
                    }
                }
