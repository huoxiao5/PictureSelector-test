...
public class VideoViewHolder extends BaseRecyclerMediaHolder {
    ...
    public VideoViewHolder(@NonNull View itemView, PictureSelectionConfig config) {
        ...
        tvTimeTag = itemView.findViewById(R.id.tv_datetime_tag);
        ...
        if (config.isDisplayTimeInGridItem){
            durationGravity = new int[2];
            durationGravity[0]=RelativeLayout.ALIGN_PARENT_START;
            durationGravity[1]=RelativeLayout.ALIGN_PARENT_TOP;
            tvTimeTag.setVisibility(View.VISIBLE);
            tvDuration.setBackgroundResource(0);
        }else{
    ...
    public void bindData(LocalMedia media, int position) {
        ...
        String time = DateUtils.getFormatString(media.getDateAddedTime()*1000);
        tvTimeTag.setText(time);
    }
}
